...
Use `gws` to create, read, edit, and manage Google Docs, with care for
structure, formatting, and final polish.
...
## Work Branding Header and Footer

If the user asks for work branding, apply both. For Muttdata, use:

- **Header:** right-aligned Muttdata logo, inserted as an inline PNG image in a
  `DEFAULT` header, `70 PT` high. Use the canonical public logo URI
  `https://drive.google.com/uc?id=1sfuWY5sM6VWTrX84x3Enj_BFUBYIX1RS&export=download`.
- **Footer:** `www.muttdata.ai`, left-aligned, 10 pt DM Sans, muted grey
  (`#666666`) in a `DEFAULT` footer.

Workflow:

1. Create missing header/footer segments with `createHeader` / `createFooter`.
2. Use the returned `headerId` / `footerId` as `segmentId` for
   `insertInlineImage`, `insertText`, `updateParagraphStyle`, and
   `updateTextStyle`.
3. Insert the logo with `objectSize.height` set to `70 PT`.
4. Keep the logo right-aligned within the header paragraph with `alignment: END`
   and compact line/paragraph spacing. Set header paragraph `indentEnd` to
   `-60 PT` to push it farther right. Set `marginHeader` to `10 PT` to keep the
   logo close to the top edge, but do not change body page margins just to
   reposition the logo.
